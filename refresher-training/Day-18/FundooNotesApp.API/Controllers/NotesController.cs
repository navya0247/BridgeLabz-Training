...
namespace FundooNotesApp.API.Controllers
{
    ...
    public class NotesController : ControllerBase
    {
        ...
        public IActionResult SetReminder(long noteId, [FromBody] DateTime reminderTime)
        {
            try
            {
                int userId = GetUserId();
                var result = _notesService.SetReminder(noteId, reminderTime, userId);
                return Ok(new ApiResponseDto<NotesResponseDto> { Success = true, Message = "Reminder set successfully", Data = result });
            }
            catch (NoteNotFoundException ex)
            {
                return NotFound(new ApiResponseDto<string> { Success = false, Message = ex.Message });
